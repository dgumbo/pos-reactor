...
public class StockReOrderHqlDevImpl implements StockReOrderHql{
    ...
    public TypedQuery<StockReOrderView> createNQuery(String stockItemName, Date dateBackAgo, int daysBackAgo, long leadDays) {
        ...
        queryString.append("\n pc.name As productCategory, sti.name As stockItem "
                ...
                + "\n 		Sum(si.quantity) /  "
                + "\n 		Case  "
                + "\n 			When DateDiff( Min(s.sellDate), CURRENT_TIMESTAMP ) = 0 Then :daysBackAgo "
                + "\n 			When DateDiff( Min(s.sellDate), CURRENT_TIMESTAMP ) < :daysBackAgo Then DateDiff( Min(s.sellDate), CURRENT_TIMESTAMP ) "
                + "\n 			Else :daysBackAgo "
                + "\n 		End ) "
                + "\n 	  * :leadDays) "
                + "\n 	  - cs.quantity ) As long) As requiredQuantity "
                + "\n  "
                ...
                + "\n 			When DateDiff( Min(s.sellDate), CURRENT_TIMESTAMP ) < :daysBackAgo Then DateDiff( Min(s.sellDate), CURRENT_TIMESTAMP ) "
                + "\n 			Else :daysBackAgo "
                + "\n 		End ) As long) As averageDailySales  "
                + "\n  "
                + "\n 	, Cast(FLOOR( "
                + "\n 		cs.quantity /  "
                ...
                + "\n 			When DateDiff( Min(s.sellDate), CURRENT_TIMESTAMP ) < :daysBackAgo Then DateDiff( Min(s.sellDate), CURRENT_TIMESTAMP ) "
                + "\n 			Else :daysBackAgo "
                + "\n 		End ) As long) As currentStockDepletionDays  "
                + "\n  "
                ...
                + "\n 			Else :daysBackAgo "
                + "\n 		End "
                + "\n 	  * :leadDays ) As long) As totalSafetyStock ");
        queryString.append("\n ) ");
        //        queryString.append("\n From Sell s "
